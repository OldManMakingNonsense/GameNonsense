...
impl Shader {
    pub fn new(vertexPath: &str, fragmentPath: &str) -> Shader {
        ...
        let mut vShaderFile =
            File::open(&Path::new(vertexPath)).unwrap_or_else(|_| panic!("Failed to open {}", vertexPath));
        let mut fShaderFile =
            File::open(&Path::new(fragmentPath)).unwrap_or_else(|_| panic!("Failed to open {}", fragmentPath));
    ...
    /// ------------------------------------------------------------------------
    pub unsafe fn setIntString(&self, name: &CStr, value: i32) {
        gl::Uniform1i(gl::GetUniformLocation(self.ID, name.as_ptr()), value);
    }
    /// ------------------------------------------------------------------------
    ...
    /// ------------------------------------------------------------------------
    pub unsafe fn setVec2(&self, name: &CStr, x: f32, y: f32) {
    ...
    unsafe fn checkCompileErrors(&self, shader: u32, type_: &str) {
        ...
        if type_ != "PROGRAM" {
            ...
            if success != gl::TRUE as GLint {
                gl::GetShaderInfoLog(shader, 2048, ptr::null_mut(), infoLog.as_mut_ptr() as *mut GLchar);
                let message = String::from_utf8_lossy(&infoLog);
                println!(
            ...
            if success != gl::TRUE as GLint {
                gl::GetProgramInfoLog(shader, 2048, ptr::null_mut(), infoLog.as_mut_ptr() as *mut GLchar);
                let message = String::from_utf8_lossy(&infoLog);
                println!(
